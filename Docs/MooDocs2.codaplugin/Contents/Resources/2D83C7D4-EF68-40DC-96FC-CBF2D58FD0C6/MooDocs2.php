...
$descPattern	   = '/\bdescription\s*\:\s*(?<desc>[\s\S]*?)\n/';
...
foreach($mooClasses as $className => $mooClass) {	
	...
	// gets description
	preg_match_all($descPattern, $mooClass['code'], $mooDesc);
	if (!empty($mooDesc[0])) {
		$mooClasses[$className]['Description'] = preg_split($csvPattern, $mooDesc['description'][0]);
	}
...
foreach($mooClasses as $className => $mooClass) {
	...
	//desc
	if (!empty($mooClass['Description'])) {
		$mooSection = sprintf($mdH3, 'Description');
		foreach($mooClass['Description'] as $key => $mooDesc) {
			$mooSection .= $mooDesc.', ';
		}
